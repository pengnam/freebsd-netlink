...
struct genl_ops {
	int (*doit)(struct mbuf *m, struct genl_info *info);
	int (*start)(struct netlink_callback *cb);
	int (*dumpit)(struct mbuf *m, struct netlink_callback *cb);
...
{
	return (
	    struct nlmsghdr *)((char *)user_hdr - GENL_HDRLEN - NLMSG_HDRLEN);
}

static inline int
genlmsg_parse(const struct nlmsghdr *nlh, const struct genl_family *family,
...
static inline void *
genlmsg_put_reply(struct mbuf *m, struct genl_info *info,
    const struct genl_family *family, int flags, uint8_t cmd)
{
	return genlmsg_put(
	    m, info->snd_portid, info->snd_seq, family, flags, cmd);
}
...
{
	nlmsg_end(
	    m, (struct nlmsghdr *)((char *)hdr - GENL_HDRLEN - NLMSG_HDRLEN));
}

static inline int
genlmsg_send_msg(struct mbuf *m, uint32_t portid)
{
...
static inline void *
genlmsg_data(struct genlmsghdr *gnlh)
{
	return ((unsigned char *)gnlh + GENL_HDRLEN);
}

static inline int
genlmsg_len(struct genlmsghdr *gnlh)
{
