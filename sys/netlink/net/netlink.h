...
/* Modified from:
 * https://elixir.bootlin.com/linux/latest/source/include/net/netlink.h
 * ========================================================================
...
int nl_register_or_replace_handler(int proto, nl_handler handle);
...
void *nl_data_end_ptr(struct mbuf *m);
...
nlmsg_put(struct mbuf *m, int portid, int seq, int type, int payload, int flags)
{
	...
	if (NLMSG_ALIGN(size) - size != 0)
		memset((char *)nlmsg_data(nlh) + payload, 0,
